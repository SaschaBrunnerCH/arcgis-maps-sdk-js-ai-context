---
name: arcgis-editing
description: Edit features with the Editor component/widget, configure forms, manage subtypes, handle attachments, and work with branch versioning. Use when adding, updating, or deleting features, building edit forms, managing feature attachments, or working with versioned edits.
---
...
1. **Editing permissions**: Calling `applyEdits` on a layer without checking edit capabilities causes silent failures.
   ...
   if (
     ...
     layer.capabilities?.editing?.supportsAddingFeatures
   ) {
     await layer.applyEdits({ addFeatures: [newFeature] });
   } else {
